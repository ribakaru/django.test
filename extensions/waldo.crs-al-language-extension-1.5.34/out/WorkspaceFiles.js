...
const ALGraphVis_1 = require("./ALGraphVis");
...
class WorkspaceFiles {
    ...
    static DoRenameFile(from, to, withGit) {
        ...
        if (!withGit || isMultiroot) {
            ...
            ApplicationInsights_1.AppInsights.getInstance().trackEvent(ApplicationInsights_1.EventName.RenameFile, appInsightsEntryProperties);
        }
        else {
            git.gitMove(from, to);
        }
    ...
    static RenameAllFiles() {
        ...
        this.getAlFilesFromCurrentWorkspace().then(Files => {
            ...
            try {
                ...
                ApplicationInsights_1.AppInsights.getInstance().trackEvent(ApplicationInsights_1.EventName.RenameAllFiles, appInsightsEntryProperties);
            }
            catch (error) {
    ...
    static ReorganizeAllFiles() {
        ...
        this.getAlFilesFromCurrentWorkspace().then(Files => {
            ...
            try {
                ...
                appInsightsEntryProperties.renamedFileCount = renamedFileCount;
                appInsightsEntryProperties.totalFileCount = totalFileCount;
                ApplicationInsights_1.AppInsights.getInstance().trackEvent(ApplicationInsights_1.EventName.ReorganizeAllFiles, appInsightsEntryProperties);
            }
            catch (error) {
                vscode.window.showErrorMessage(error.message);
            }
    ...
    static getObjectTypeFolder(navObject, mySettings) {
        if (navObject.objectCodeunitSubType) {
            if (navObject.objectCodeunitSubType.toLowerCase() == 'test') {
                return '';
            }
        }
        if (mySettings[Settings_1.Settings.ReorganizeByNamespace]) {
            let directoryPath = path.join(...navObject.objectNamespace.split("."));
            if (mySettings[Settings_1.Settings.NamespacePrefixToIgnore]) {
                directoryPath = path.join(...navObject.objectNamespace.replace(new RegExp(`^${mySettings[Settings_1.Settings.NamespacePrefixToIgnore]}\\.?`), "").split("."));
            }
    ...
    static CreateGraphVizDependencyGraph() {
        return __awaiter(this, void 0, void 0, function* () {
            crsOutput.showOutput(`Creating dependency graph (GraphViz) from apps in this workspace`, true);
            let GraphVis = new ALGraphVis_1.ALGraphVis();
            let appJsonFiles = yield vscode.workspace.findFiles('**/app.json');
            appJsonFiles.forEach(appJsonFile => {
                let appJson = new AppJson_1.AppJson(appJsonFile);
            ...
            yield this.showNewUntitledDocument(GraphVis.dotContent).then(() => {
